...
function upgradeDynamicPageLayout(stored) {
  ...
  if (storedPages.length === 7 && storedPages.every((page) => page.layoutVersion >= 4)) return storedPages;
  try {
...
function renderSourceCard(source) {
  ...
  return `<article class="source-card ${source.confirmed ? "is-confirmed" : ""}">
    <div class="source-card-heading"><div><span class="source-domain">${escapeHtml(source.domain)}</span><a href="${escapeHtml(source.url)}" target="_blank" rel="noreferrer">${escapeHtml(source.title)}</a></div><span class="status-pill ${tone}">${label}</span></div>
