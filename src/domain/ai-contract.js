...
export function createOutlineRequest(topic, research, viewpointId, taskId = createTaskId("outline")) {
  ...
  return {
    ...
    input: {
      ...
      copyPolicy: "七页必须承担不同任务，每个 keyPoint 都要给出具体对象、条件或行动，优先使用已确认的英雄、羁绊、装备和运营阶段。禁止在多个小节反复使用同一段收尾句，也禁止只替换名词的模板文案。语言像熟悉游戏的朋友分享：自然、直接、有判断依据，保留少量小红书式提醒，但不要堆“宝子”“绝绝子”等夸张网络词。",
